...
class Config:
    ...
    file_name: str = "CIC-ToN-IoT"
    data_dir: str = "./data"
    ckpt_dir: str = "CIC_TON_IOT/checkpoints"
...
def load_data(cfg: Config):
    ...
    bin_labels = data_all["attack"].cpu().numpy()
    _print_label_distribution("attack (binary)", bin_labels)
...
def run_once(cfg: Config):
    ...
    for epoch in range(1, cfg.tune_epochs + 1):
        ...
        if test_metrics["f1_mac"] > best_test_f1:
            best_test_f1 = test_metrics["f1_mac"]
            best_epoch = epoch
            best_thr = t_star
