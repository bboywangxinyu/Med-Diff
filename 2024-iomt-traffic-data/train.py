"""Train Med_Diff on 2024 IoMT traffic data."""
...
class Config:
    ...
    file_name: str = "2024-iomt-traffic-data"
    data_dir: str = "./data"
    ckpt_dir: str = "2024-iomt-traffic-data/checkpoints"
...
def load_data(cfg: Config):
    ...
    if "attack" in data_all:
        data_all["attack"] = (data_all["attack"] != 5).long()
...
def run_once(cfg: Config):
    ...
    pairs = sorted(zip(uniq, cnt), key=lambda x: x[1], reverse=True)

    print("    label : count (ratio)")
    for k, v in pairs:
        print(f"      {int(k):>3d} : {int(v):>8d} ({v/total*100:.2f}%)")
